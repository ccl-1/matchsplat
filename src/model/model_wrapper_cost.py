...
class ModelWrapper(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        for i in range(gaussians.means.shape[0]):
            ...
            save_points_ply(gs_i,f"outputs/tmp/cost/iproj_worldf{i}.ply")
        
        
        i=0
        img0 = batch['context']['image'][i, 0]
        img1 = batch['context']['image'][i, 1]
        save_image(img0, f"outputs/tmp/cost/img0.png")
        save_image(img1, f"outputs/tmp/cost/img1.png")
        ...
        save_points_ply(pts3d_world.reshape(-1, 3), f"outputs/tmp/cost/iproj_world{i}_rendered_1.ply")
        pts3d_world = origins[1:] + directions[1:] * depth_combined[1:][..., None]
        save_points_ply(pts3d_world.reshape(-1, 3), f"outputs/tmp/cost/iproj_world{i}_rendered_2.ply")

        pcd1 = o3d.io.read_point_cloud(f"outputs/tmp/cost/iproj_world{i}_rendered_1.ply")
        pcd2 = o3d.io.read_point_cloud(f"outputs/tmp/cost/iproj_world{i}_rendered_2.ply")
        pcd1 = pcd1.paint_uniform_color([1, 0, 0])  #red
        pcd2 = pcd2.paint_uniform_color([0, 0, 1])  
        pcd_combined = pcd1 + pcd2 
        o3d.io.write_point_cloud(f"outputs/tmp/cost/iproj_world{i}_rendered_combined.ply", pcd_combined)
        input()
