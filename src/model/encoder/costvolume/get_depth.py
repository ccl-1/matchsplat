...
class DepthPredictorMultiView(nn.Module):
    ...
    def forward(
        ...
        """
            features  torch.Size([b, v, 128, 64, 64])
            intrinsics  torch.Size([b, v, 3, 3])
        ...
        if self.wo_cost_volume_refine:
            ...
            raw_correlation = self.corr_refine_net(raw_correlation_in)  # (vb d_candi h w) 
        ...
        pdf_max = torch.max(pdf, dim=1, keepdim=True)[0]  # argmax (vb, 1, 64, 64)
        pdf_max = F.interpolate(pdf_max, scale_factor=self.upscale_factor) # (vb, 1, 256, 256)
        ...
        raw_gaussians_in = torch.cat(raw_gaussians_in, dim=1) # (vb, 128 + 64 + 3, 256, 256)
